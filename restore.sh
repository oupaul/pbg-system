...
if [ -d "${PROJECT_DIR}/migrations" ]; then
    log "執行增量遷移..."
    # 改為統一呼叫 runner.js（單一事實來源，與 deploy.sh 一致），
    # 避免像過去那樣手動維護清單、新增 migration 卻忘了補進還原腳本，
    # 導致還原較舊備份後缺少後續版本新增的資料表
    node migrations/runner.js || warning "部分 migration 執行失敗，請檢查上方輸出並視需要手動執行 node migrations/runner.js"
    log "✓ 資料庫遷移完成"
else
