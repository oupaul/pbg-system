...
- 專案明細：專案編號、客戶、專案名稱、類型（顏色與專案管理一致）、毛利率、毛利、收入、成本、簽約年度、狀態
- **毛利警示閾值**：可於專案類型管理設定，針對每個專案所屬類型判斷，低於閾值時顯示警示
- 依業務彙總：各業務專案數、總收入、總成本、總毛利
- 依類型彙總：各專案類型的收入、成本、毛利統計
- **年度與狀態篩選**：全部年度／指定年度；全部／未結案／已結案，可同時使用，重設按鈕一鍵清空
- **Excel、PDF 匯出**：支援依篩選年度與狀態匯出（四張工作表或專案明細表），PDF 表格置中、標題分頁、換頁重複表頭
- **手機版**：篩選與按鈕響應式換行，Excel/PDF 按鈕小螢幕僅顯示圖示
- 僅 admin、user 可見
...
- **專案毛利分析** - 支援 Excel、PDF 匯出（年度與狀態篩選，四張工作表／專案明細表）
- 支援合併儲存格處理
- 自動資料驗證與錯誤提示
...
- 專案毛利分析 PDF（橫向 A4，表格置中、標題分頁、換頁重複表頭、標題置中對齊）
...
### 2026-02-12 - 專案毛利分析功能增強

#### 狀態篩選

- ✅ 新增「未結案」「已結案」狀態篩選，與年度篩選可同時使用
- ✅ Excel／PDF 匯出支援 `&status=未結案` 或 `&status=已結案`，檔名帶入狀態

#### 重設按鈕

- ✅ 篩選列新增「重設」按鈕，一鍵清空年度與狀態篩選

#### PDF 匯出版面調整

- ✅ 依群組彙總搬至專案明細上方
- ✅ 欄位寬度優化，數值完整顯示（收入、成本、毛利、毛利率）
- ✅ 專案明細列高 40pt，容納專案名稱換行
- ✅ 相關檔案：`src/services/PdfExportService.js`、`src/views/gross-profit/index.ejs`
...
### 2026-02-13 - 專案毛利分析欄位與 PDF 優化

#### 欄位與視覺調整

- ✅ 專案明細欄位順序：專案編號、客戶、專案名稱、類型、毛利率、毛利、收入、成本、簽約年度、狀態
- ✅ 類型標籤顏色與專案管理一致（從 `project_types.badge_color`）
- ✅ 客戶欄位：從 `customers` 表 LEFT JOIN 取得
- ✅ 相關檔案：`GrossProfitAnalysisService.js`、`grossProfit.js`、`gross-profit/index.ejs`、`ExcelExportService.js`

#### PDF 匯出優化

- ✅ 表格置中、每頁標題與頁碼、換頁時重複表格標題列
- ✅ 表格標題文字水平與垂直置中對齊
- ✅ 相關檔案：`src/services/PdfExportService.js`

#### 手機版／響應式

- ✅ 篩選與按鈕小螢幕換行（flex-wrap）
- ✅ Excel、PDF 按鈕在 <768px 僅顯示圖示，避免超出版面
- ✅ 相關檔案：`public/css/style.css`、`gross-profit/index.ejs`
